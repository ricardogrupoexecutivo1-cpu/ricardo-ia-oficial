...
export async function getAgroListings(): Promise<AgroListingFromDb[]> {
  ...
  if (listingsError) {
    console.error("Erro ao buscar listings do AGRO:", listingsError);
    return [];
  }
  ...
  const [{ data: companiesData, error: companiesError }, { data: contactsData, error: contactsError }, { data: locationsData, error: locationsError }] =
    await Promise.all([
      supabase
        .from("companies")
        .select(
  ...
  if (companiesError) {
    console.error("Erro ao buscar companies do AGRO:", companiesError);
  }

  if (contactsError) {
    console.error("Erro ao buscar contacts do AGRO:", contactsError);
  }

  if (locationsError) {
    console.error("Erro ao buscar locations do AGRO:", locationsError);
  }
