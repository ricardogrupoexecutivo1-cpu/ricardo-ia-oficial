...
const ACTIVITIES_TABLE = "finance_activities";
...
async function resolveCompanyId(
  ...
  return {
    companyId: null,
    error:
      "Nenhuma empresa encontrada para o e-mail informado. Cadastre ou vincule a empresa antes de criar atividades.",
  };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const activitiesResult = await supabase
      ...
      .select(
        "id, company_id, category_id, name, color, description, is_active, sort_order, created_at, updated_at",
      )
    ...
    return json({
      ...
      items: activitiesResult.data satisfies ActivityRow[],
      message:
        "Atividades financeiras carregadas com sucesso. Sistema em constante atualização e pode haver momentos de instabilidade durante melhorias.",
    });
  } catch (error) {
    return json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Erro interno ao carregar atividades.",
      },
      500,
    );
  }
  ...
  try {
    ...
    if (existingActivity.error) {
      return json(
        {
          ok: false,
          error: `Erro ao validar atividade existente: ${existingActivity.error.message}`,
        },
        500,
      );
    }
    ...
    if (existingActivity.data?.id) {
      return json(
        {
          ok: false,
          error: "Já existe uma atividade com esse nome para esta empresa.",
          existingId: existingActivity.data.id,
        },
        409,
      );
    }
    ...
    return json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Erro interno ao criar atividade.",
      },
      500,
    );
  }
